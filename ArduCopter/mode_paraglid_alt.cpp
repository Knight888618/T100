...
bool ModeParaglidAlt::init(bool ignore_checks)													// loiter_init - initialise loiter controller
{
	...
	_sen_angle_min = copter.g2.user_parameters.get_sen_angle_min();
	_sen_angle_max = copter.g2.user_parameters.get_sen_angle_max();
	...
	_climb_rate_max = copter.g2.user_parameters.get_climb_rate_max();
	_climb_rate_min = copter.g2.user_parameters.get_climb_rate_min();
	...
	_target_pitch_angle = copter.g2.user_parameters.get_ctrl_angle0();
	...
	gcs().send_text(MAV_SEVERITY_NOTICE, "Enter Mode 51");

    return true;
}

void ModeParaglidAlt::run()																		// loiter_run - runs the loiter controller, should be called at 100hz or more
{
    ...
    attitude_control->input_euler_angle_roll_pitch_euler_rate_yaw(target_roll, constrain_float(target_pitch, -_pitch_max, _pitch_max), target_yaw_rate);// call attitude controller
